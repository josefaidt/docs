...
const { data: org } = await client.models.Organization.get({
  id: 'AE6191BF-FEB4-4D8B-867D-898A7DA33101'
});
...
<Callout>
  **Note:** The `belongsTo()` method requires that a `hasOne()` or `hasMany()`
...
<Callout warning>
  "Belongs to" relationships **do not** allow you to update both sides of a "has
  one" and "belongs to" relationship in a transactional manner.
</Callout>
...
const projectWithTeam = await client.models.Project.get(
  { id: '1234' },
  { selectionSet: ['team.*'] }
);
