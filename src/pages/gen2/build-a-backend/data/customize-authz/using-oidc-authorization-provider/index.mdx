...
Private, owner, and group authorization can be configured with an OpenID Connect (OIDC) authorization mode. Add `"oidc"` to the authorization rule as the provider. Use the `oidcAuthorizationMode` property to configure the _OpenID Connect provider name_, _OpenID Connect provider domain_, _Client ID_, _Issued at TTL_, and _Auth Time TTL_.
