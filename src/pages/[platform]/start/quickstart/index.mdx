...
Go to the **src/todos/todos.component.ts** file and add a new `deleteTodo` function.
...
```html title="src/todos/todos.component.html"
...
  <ul>
...
- A Flutter version higher than 3.3.0
...
Update the `createTodo` function in the `MyAmplifyAppApp.swift` file with the following code:
...
Next, update the `listTodos` function in the `MyAmplifyAppApp.swift` for listing to-do items:
...
### 7. Implement per-user authorization
