...
Note that **@versioned** is only supported in client code (statement and types) generated via AppSync [codegen](/[platform]/tools/cli-legacy/client-codegen/). **@versioned** is not supported by models generated via `amplify codegen models`. Use [Amplify DataStore](/[platform]/build-a-backend/more-features/datastore/set-up-datastore/) instead of **@versioned** to provide offline app data access with built-in conflict-resolution.
