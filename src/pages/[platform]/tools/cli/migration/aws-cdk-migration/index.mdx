...
2. For projects using `amplify add custom`: Update `package.json` for `custom` categories in `amplify/backend/custom/<custom-resource-name>/package.json` as shown below:
   ...
   ![Amplify overrides warning](/images/ts-guide-cdk-package.png) And update `amplify/backend/custom/<custom-resource-name>/cdk-stack.ts` for `custom` categories as shown below:
