...
export const meta = {
  ...
  description:
    'Learn how you can import existing S3 bucket or DynamoDB table resources as a storage resource for other Amplify categories (API, Function, and more) using the Amplify CLI.',
  platforms: [
    'android',
    'angular',
    'flutter',
...
If you're using an imported S3 bucket with an imported Cognito resource, then you'll need to update the policy of your Cognito Identity Pool's authenticated and unauthenticated role. Create new **managed policies** (not _inline policies_) for these roles with the following statements:
